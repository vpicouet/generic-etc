"""
Test validation function on a single instrument (GALEX FUV)
"""
import numpy as np
import matplotlib
...
print(f"\nTesting validation on: {instrument_name}")
...
# Test simple SNR calculation
print("\n1. Testing simple SNR calculation...")
...
snr_generic = obs_generic.SNR[obs_generic.i]
...
snr_astropy = result_astropy['SNR']

rel_diff = 100 * (snr_generic - snr_astropy) / snr_astropy

print(f"  Generic ETC SNR: {snr_generic:.4f}")
print(f"  Astropy SNR: {snr_astropy:.4f}")
print(f"  Relative difference: {rel_diff:.2f}%")

# Test exposure time variation
print("\n2. Testing exposure time variation...")
